...
class Predictor():
    ...
    def predict(self, a3m_fn, out_prefix,length, hhr_fn=None, atab_fn=None, window=150, shift=75):
        ...
        l1 = torch.arange(length).long().view(1, length)
        l2 = torch.arange(length+200,L+200).long().view(1, L-length)
        idx_pdb= torch.cat((l1,l2),1)
        seq = msa[:,0]
        #
        # template features
...
def get_args():
    ...
    parser.add_argument("--length","-l", dest='length', default=0,required=True,type=int)
...
if __name__ == "__main__":
    ...
    if not os.path.exists("%s.npz"%args.out_prefix):
        ...
        pred.predict(args.a3m_fn, args.out_prefix, args.length, args.hhr, args.atab)
