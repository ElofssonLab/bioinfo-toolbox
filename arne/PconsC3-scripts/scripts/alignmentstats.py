...
def cluster(a, threshold):
	if threshold < 0.7:
		b = subprocess.check_output('~/sw/cd-hit-v4.5.4-2011-03-07/cd-hit -n 3 -i ' + a + ' -c {:6.4f}'.format(threshold) + ' -o ' + a + '.cd{:d}'.format(int(threshold*100)), shell=True)
		subprocess.call('rm ' + a + '.cd{:d}'.format(int(threshold*100)), shell=True)
	else:
		b = subprocess.check_output('~/sw/cd-hit-v4.5.4-2011-03-07/cd-hit -i ' + a + ' -c {:6.4f}'.format(threshold) + ' -o ' + a + '.cd{:d}'.format(int(threshold*100)), shell=True)
		subprocess.call('rm ' + a + '.cd{:d}'.format(int(threshold*100)), shell=True)
