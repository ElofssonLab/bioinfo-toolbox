...
#
# Modified by Arne Elofsson to limit the maximum number of sequences in the file.
#
...
Options:
  ...
  -max      maximum number of sequences included in the output (default 0=unlimited)
...
################################################################################################
    ...
    if ($maxseq > 0 && $n > $maxseq) {
	$jump=$n/$maxseq;
#	print "Too many sesuqences only including $maxseq out of $n, i.e. skipping every $jump sequence\n";
    }
    ...
    my $nquery=-1;
#    for ($k=0; $k<$n; $k++) {
    for (my $count=0; $count<$n; $count+=$jump) {
	$k=int($count);
...
#	    for ($k=0; $k<scalar(@names); $k++) {       # go through all sequences
	    for (my $count=0; $count<scalar(@names); $count+=$jump) {
		$k=int($count);
		$names[$k] =~ s/\s*(\S+).*/$1/;
...
#	for ($k=0; $k<$n; $k++) {
	...
	if ($nin==1) {print("Reformatted $infile with 1 sequence from $informat to $outformat and written to file $outfile\n");}
	    ...
	    {
		print("Reformatted $infile with $maxseq (out of total $n) sequences from $informat to $outformat and written to file $outfile\n");
	    }
	}
    }
