...
else
{
    ...
    close(OUTFILE);
    close(INFILE);
    &HHPaths::System("rm $tmpfile.mtx $tmpfile.ss");
...
sub RunPsipred() {
    ...
    if ($v<=3) { unlink(split ' ', "$tmpfile.pn $tmpfile.sn $tmpfile.mn $tmpfile.chk $tmpfile.blalog $tmpfile.mtx $tmpfile.aux $tmpfile.ss $tmpfile.sq");}
    return;
}
