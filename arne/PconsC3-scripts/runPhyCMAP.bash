...
(cd $epadCaDir; ./bin/EPADCalc -e EPAD -A $workdir/$pdbid.pretag.ca -F 11 | sed -E "s/\*//g" > $workdir/$pdbid.epadca.prob ; exit $? ) &> epadca.log
if [ $? -ne 0 ] ;then echo "ERR4 $error_epad" ; exit -1 ;fi

(cd $epadCbDir; ./bin/EPADCalc -e EPAD -A $workdir/$pdbid.pretag.cb -F 11 | sed -E "s/\*//g" > $workdir/$pdbid.epadcb.prob ; exit $? ) &> epadcb.log
