...
sub process_parameters{
	...
	confess "ERROR! Fasta file $file_top does not exist!" if ($file_top and not -f $file_top);
	...
	system_cmd("cp $file_top    $dir_out/input/$id.top") if $file_top;
...
# First we need to merge ss and top files..
	...
	if ($file_top){
		my $a = seq_fasta($file_top);
		my $b = seq_fasta($file_fasta);
		confess "ERROR! Fasta and top sequence lengths do not match!\nFasta:$b\nSS   :$a" if length($a) != length($b);
	}
	if ($file_rr){
...
#	print JOB "source /home/x_arnel/git/bioinfo-toolbox/arne/CONFOLD/cns_solve_env.sh       \n";
    ...
    #wait for $pid;
    my @commands = ("chmod +x $dir_out/$stage/job.sh");
    my @pids;
    foreach my $cmd( @commands ){
        my $pid = fork;
        if ( $pid ) {
    ...
    wait for @pids;
    @commands = ("$dir_out/$stage/job.sh > job.log");
    @pids=();
    foreach my $cmd( @commands ){
        my $pid = fork;
        if ( $pid ) {
