...
Disorder<-round(dat$Disorder)
TM<-dat$TM
PDB<-round(as.numeric(dat$PDB_ID)-1)
PFAM<-round(as.numeric(dat$PFAM_ID)-1)
...
# Ja
for (i in loop){
    NumAll[i]=length(which(dat$Pfam_Meff > cutoffs[i] & dat$Pfam_Meff <= cutoffs[i+1]))
    NumTM[i]=length(which(dat$Pfam_Meff > cutoffs[i] & dat$Pfam_Meff <= cutoffs[i+1] & dat$TM > 0))
    NumNoTM[i]=length(which(dat$Pfam_Meff > cutoffs[i] & dat$Pfam_Meff <= cutoffs[i+1] & dat$TM == 0))
    NumDiso[i]=length(which(dat$Pfam_Meff > cutoffs[i] & dat$Pfam_Meff <= cutoffs[i+1] & dat$Disorder>=0.5))
    NumNoDiso[i]=length(which(dat$Pfam_Meff > cutoffs[i] & dat$Pfam_Meff <= cutoffs[i+1] & dat$Disorder<0.5))
    NumPDB[i]=length(which(dat$Pfam_Meff > cutoffs[i] & dat$Pfam_Meff <= cutoffs[i+1] & dat$PDB_ID != ""))
    NumNoPDB[i]=length(which(dat$Pfam_Meff > cutoffs[i] & dat$Pfam_Meff <= cutoffs[i+1] & dat$PDB_ID == ""))
    NumSeg[i]=length(which(dat$Pfam_Meff > cutoffs[i] & dat$Pfam_Meff <= cutoffs[i+1] & dat$Seg_low > 0))
    NumNoSeg[i]=length(which(dat$Pfam_Meff > cutoffs[i] & dat$Pfam_Meff <= cutoffs[i+1] & dat$Seg_low == 0))
    labels[i]=cutoffs[i]
...
pct <- round(NumAll/sum(NumAll)*100,digits=1)
names <- paste(labels,pct)
names <- paste(names,"%",sep="")
iniR=1

outfile=paste(genome,"-PDB-pie.png",sep="")
fraction=sum(NumPDB)/(sum(NumNoPDB)+sum(NumPDB))
png(outfile)
pie(NumAll, labels=names,col=colors,main=genome,radius=iniR,border = NA)
floating.pie(0,0,NumPDB, col=colors,main="PDB",radius=fraction)
floating.pie(0,0,NumNoPDB, col=colors,main='',radius=(1-fraction))
dev.off()

outfile=paste(genome,"-TM-pie.png",sep="")
fraction=sum(NumTM)/(sum(NumNoTM)+sum(NumTM))
png(outfile)
pie(NumAll, labels=names,col=colors,main=genome,radius=iniR,border = NA)
floating.pie(0,0,NumTM, col=colors,main="TM",radius=fraction)
floating.pie(0,0,NumNoTM, col=colors,main='',radius=(1-fraction))
dev.off()

outfile=paste(genome,"-Diso-pie.png",sep="")
fraction=sum(NumDiso)/(sum(NumNoDiso)+sum(NumDiso))
png(outfile)
pie(NumAll, labels=names,col=colors,main=genome,radius=iniR,border = NA)
floating.pie(0,0,NumDiso, col=colors,main="Diso",radius=fraction)
floating.pie(0,0,NumNoDiso, col=colors,main='',radius=(1-fraction))
dev.off()

#outfile=paste(genome,"-TM-seg.png",sep="")
#fraction=sum(NumSeg)/(sum(NumNoSeg)+sum(NumSeg))
#png(outfile)
#pie(NumAll, labels=names,col=colors,main=genome,radius=iniR,border = NA)
#floating.pie(0,0,NumSeg, col=colors,main="Seg",radius=fraction)
#floating.pie(0,0,NumNoSeg, col=colors,main='',radius=(1-fraction))
#dev.off()




                                        #floating.pie(0,0,NumTM, col=rainbow(length(NumAll)),main=genome,radius=4*iniR)
#floating.pie(0,0,NumTM, col=rainbow(length(NumAll)),main=genome,radius=4*iniR)

#legend(0, 5*iniR, labels, col=colors,pch=19,bty='n', ncol=2)
