...
Meff500=NULL
    ...
    for (i in loop){
        frac[i]=0.1*i
        disofrac[i]=length(which(dat$Disorder > (i-1)*0.1 & dat$Disorder < (i)*0.1  & dat$Pfam_ID != ""))/length(which(dat$Disorder > (i-1)*0.1 & dat$Disorder < (i)*0.1))
    ...
    TMPFam=NULL
    TMPFam[1]=length(which(dat$TM_topogy== "M"  & Pfam_ID != "" ))/length(which(dat$TM_topogy== "M"))
    TMPFam[2]=length(which(dat$TM_topogy== "i"  & Pfam_ID != "" ))/length(which(dat$TM_topogy== "i"))
    TMPFam[3]=length(which(dat$TM_topogy== "o"  & Pfam_ID != "" ))/length(which(dat$TM_topogy== "o"))
