...
def gen_rst(npz, tmpdir, params): #def gen_rst(params):
...
def add_rst_chain2(pose, rst, sep1, sep2, params, nogly=False, use_orient=None, pcut=None, p12_cut=0.0):
    if use_orient == None:
        use_orient = params['USE_ORIENT']
    if pcut == None:
        pcut=params['PCUT']
    ...
    if nogly==True:
        intrachain = [line for a,b,p,line in rst['intrachain'] if seq[b]!='G' and seq[a]!='G' and seq[b]!='G' and p>=pcut]
        if inter:
            dist_r = [r for a,b,p,p_12,r in rst['dist'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and seq[a]!='G' and seq[b]!='G' and p>=pcut and p_12>=p12_cut]
            if use_orient:
                omega_r = [r for a,b,p,r in rst['omega'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and seq[a]!='G' and seq[b]!='G' and p>=pcut+P_ADD_OMEGA] #0.5
            ...
            dist_r = [r for a,b,p,p_12,r in rst['dist'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and seq[a]!='G' and seq[b]!='G' and p>=pcut and p_12>=p12_cut and np.sign(a-seqlen)==np.sign(b-seqlen)]
            if use_orient:
                omega_r = [r for a,b,p,r in rst['omega'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and seq[a]!='G' and seq[b]!='G' and p>=pcut+P_ADD_OMEGA and np.sign(a-seqlen)==np.sign(b-seqlen)] #0.5
                theta_r = [r for a,b,p,r in rst['theta'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and seq[a]!='G' and seq[b]!='G' and p>=pcut+P_ADD_THETA and np.sign(a-seqlen)==np.sign(b-seqlen)] #0.5
                phi_r   = [r for a,b,p,r in rst['phi'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and seq[a]!='G' and seq[b]!='G' and p>=pcut+P_ADD_PHI and np.sign(a-seqlen)==np.sign(b-seqlen) ] #0.
    else:
        intrachain = [line for a,b,p,line in rst['intrachain'] if seq[b]!='G' and seq[a]!='G' and seq[b]!='G' and p>=pcut]                
        if inter:
            dist_r = [r for a,b,p,p_12,r in rst['dist'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and p>=pcut and p_12>=p12_cut]
            ...
            dist_r = [r for a,b,p,p_12,r in rst['dist'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and p>=pcut and p_12>=p12_cut  and np.sign(a-seqlen)==np.sign(b-seqlen) ]
            if use_orient:
                omega_r = [r for a,b,p,r in rst['omega'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and p>=pcut+P_ADD_OMEGA  and np.sign(a-seqlen)==np.sign(b-seqlen) ]
                theta_r = [r for a,b,p,r in rst['theta'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and p>=pcut+P_ADD_THETA  and np.sign(a-seqlen)==np.sign(b-seqlen)  ]
                phi_r   = [r for a,b,p,r in rst['phi'] if abs(a-b)>=sep1 and abs(a-b)<sep2 and p>=pcut+P_ADD_PHI  and np.sign(a-seqlen)==np.sign(b-seqlen) ] #0.6
    ...
    with open(tmpname,'w') as f:
        for line in intrachain:
            f.write(line+'\n')
        f.close()

    # add to pose
    intraconstraints = rosetta.protocols.constraint_movers.ConstraintSetMover()
    #constraints.constraint_set(cset)
    intraconstraints.constraint_file(tmpname)
    intraconstraints.add_constraints(True)
    intraconstraints.apply(pose)
