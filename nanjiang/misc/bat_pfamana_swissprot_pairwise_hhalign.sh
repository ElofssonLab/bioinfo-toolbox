...
Description:
    pfam TM evolution analysis script just for wk/MPTopo/pfamAna_swissprot
...
Created 2014-09-11, updated 2014-09-11, Nanjiang Shu
"
...
all_seqdb=$DATADIR3/data/uniprot/swissprot/20140827/uniprot_sprot.nr100.fasta
rootdir=$DATADIR3/wk/MPTopo/pfamAna_swissprot/
...
case $seqtype in 
    ...
    sw*) seqtype=swissprot; seqtype_addname=;; #this is already for swissprot
...
predtopodir=$rootdir
bsname_topcons_single=Pfam27-A.perTM75_nseq4
bsname_topcons=Pfam27-A.perTM75_nseq4.topcons.result
...
case $foldername in 
    ...
    topcons2_agreement*)
        ...
        #topofile=$predtopodir/pred_topcons2/${bsname_topcons2}_TOPCONS_filterSP.m1.agree-${topo_agreement_str}.topo
        topofile=$predtopodir/pred_topcons2/${bsname_topcons2}_TOPCONS_filterSP.topo
        rltyFile=$predtopodir/pred_topcons/${bsname_topcons2}_TOPCONS.rlty
        ;;
    topcons2s_agreement*)
        TYPE=TOPCONS2
        topo_agreement_str=`echo $foldername | sed 's/topcons2s_agreement_//'`
        #topofile=$predtopodir/pred_topcons2/${bsname_topcons2}_TOPCONS_filterSP.m1.agree-${topo_agreement_str}.topo
        topofile=$predtopodir/pred_topcons2/${bsname_topcons2}_TOPCONS_filterSP.topo
        rltyFile=$predtopodir/pred_topcons/${bsname_topcons2}_TOPCONS.rlty
        ;;
...
signalpfile=$rootdir/pred_signalp/Pfam27-A.perTM75_nseq4.signalp_list
...
#=====================================
uniqidlistfile=$rootdir/pfammap_from_uniprot/Pfam27-A-full.seqfrompfamfasta_nr100.percentTMpro_scampi.perTM75_nseq4${addname}.uniq.pfam.seqidlist

tm_pfamid2seqidFile=$rootdir/pfammap_from_uniprot/Pfam27-A-full.seqfrompfamfasta_nr100.percentTMpro_scampi.perTM75_nseq4${addname}.pfamid2seqid

tm_seqid2pfamidFile=$rootdir/pfammap_from_uniprot/Pfam27-A-full.seqfrompfamfasta_nr100.percentTMpro_scampi.perTM75_nseq4${addname}.seqid2pfamid

tm_pfamidListFile=$DATADIR3/data/pfam/pfam27.0/Pfam27-A-full.seqfrompfamfasta_nr100.percentTMpro_scampi.perTM75_nseq4${addname_pfamtype}.pfamidlist

tm_clanidListFile=$DATADIR3/data/pfam/pfam27.0/Pfam27-A-full.seqfrompfamfasta_nr100.percentTMpro_scampi.perTM75_nseq4${addname_pfamtype}.clanidlist

pfamDefFile=$DATADIR3/data/pfam/pfam27.0/Pfam-A.clans.tsv
#======================================
...
if [ "$TYPE" == "TOPCONS" ]; then
    tmprolistfile=$DATADIR3/wk/MPTopo/pfamAna_swissprot/pred_topcons/Pfam27-A.perTM75_nseq4.topcons.result_TOPCONS.m1.agree-${topo_agreement_str}.tmproidlist
elif [ "$TYPE" == "TOPCONS_SINGLE" ]; then
    tmprolistfile=$DATADIR3/wk/MPTopo/pfamAna_swissprot/pred_topcons_single_method4/Pfam27-A.perTM75_nseq4_topcons_single.m1.agree-${topo_agreement_str}.tmproidlist
    ...
    tmprolistfile=$DATADIR3/wk/MPTopo/pfamAna_swissprot/pred_topcons2/${bsname_topcons2}_TOPCONS_filterSP.tmproidlist
    signalpfile=$DATADIR3/wk/MPTopo/pfamAna_swissprot/pred_topcons2/${bsname_topcons2}_TOPCONS.sp_list
fi
...
for alignrange in all full; do
    ...
    exec_cmd "$binpath/ana_paircmp_result.py $paircmpfile -outpath $rstpairpath -tableinfo $tableinfofile -dgscore $dgscorefile -mp $pairwise_comparison_method -topoaln $topoalnfile -rltyinfo $rltyFile -signalp $signalpfile  -seqaln $pairalnfile  -seqidttype 1 -alignrange $alignrange -dupfile $dupfile -printcountpair -seq2fammap $tm_seqid2pfamidFile -seqid2pfamid $tm_seqid2pfamidFile"
done

exec_cmd "$binpath/tmp_ana_numTMHeatMap_pfam.py $paircmpfile -clanidlist $tm_clanidListFile -pfamidlist $tm_pfamidListFile -pairlistwithpfamid $pairlistWithPfamIDFile -seqid2pfamid $tm_seqid2pfamidFile -pfamid2seqid $tm_pfamid2seqidFile -predTMdbname $topofile -pfamdef $pfamDefFile -signalp $signalpfile -pfamtype $pfamtype -mp $mp -winsize 50 -o t1.tmp.txt -outname ${outname}.tmp_TMHeatMap"
