#!/bin/bash
# Description:
# Scanning in the homologous proteins of the query protein, check whether there
# are membrane proteins with different topologies among the homologous proteins
# to the query
...
usage="
Usage:  scanDIFFTopo.sh <fasta-seq>
Options:
    -l          <file> : set the file containing ids (rootname) of sequences
    --aapath    <path> : set the path for amino acid sequence files, if supplied, filename=\$aaPath/\$id.aa
    --blastpath <path> : set the path for blast files (output with m=9 option),if supplied, filename=\$blastPath/\$id.blast
    --blastdb   <str>  : set the blastdb, default=$blastdb
    --blastbin  <str>  : set the blast executable path, default=$blastbin
...
#}}}
function AnaMSATopo3_single() # $id#{{{
{
    ###### using position specific gap opens for alignment 
    ###### the gapopen arrays are set by the sliding DG values instead of the predicted topology
...
#     awk '/^[^#]/{if($12>=30 && $16 >= 5.8 && $6 != "OK" && $18=="yes") print}' $topoCmpHMMScoreFile | sort -k12,12rg -k16,16rg> $outpath/$id.blast.result
    ...
    echo "The result has been output to \"$outpath/$id.sel.topoalnwithdgscore\""

    if [ "$isClean" == "true"  ]; then
        /bin/rm -f $outpath/$id.blast.dg
        /bin/rm -f $outpath/$id.blast.dgscan
        /bin/rm -f $outpath/$id.blast.fa
        /bin/rm -f $outpath/$id.blast.fa.dgscorelist
        /bin/rm -f $outpath/$id.blast.fa.res
        /bin/rm -f $outpath/$id.blast.fa.topo
        /bin/rm -f $outpath/$id.blast.fa.topowithdgscore
        /bin/rm -f $outpath/$id.blast.fawithgapopen
        /bin/rm -f $outpath/$id.blast.fa.xml.res
        /bin/rm -f $outpath/$id.blast.helixfrag
        /bin/rm -f $outpath/$id.blast.needle
        /bin/rm -f $outpath/$id.blast.topocmp.anafreq
        /bin/rm -f $outpath/$id.blast.topocmp.anafreq.png
        /bin/rm -f $outpath/$id.blast.topocmp_and_hmmscore
        /bin/rm -f $outpath/$id.dg
        /bin/rm -f $outpath/$id.dgscan
        /bin/rm -f $outpath/$id.dgscorelist
        /bin/rm -f $outpath/$id.fa
        /bin/rm -f $outpath/$id.fawithgapopen
        /bin/rm -f $outpath/$id.helixfrag
        /bin/rm -f $outpath/$id.hmmscorelist
        /bin/rm -f $outpath/$id.topo
        /bin/rm -f $outpath/$id.topoaln
        /bin/rm -f $outpath/$id.topocmp
        /bin/rm -f $outpath/$id.topocmp_and_hmmscore.topoalnwithdgscore
        /bin/rm -f $outpath/$id.topocmp.log
        /bin/rm -f $outpath/$id.topowithdgscore
    fi
}
#}}}
...
errFile=/tmp/scanDIFFTopo.$$.$RANDOM.err
idListFile=
aaPath=
blastPath=
outpath=
...
if [ "$aaSeqFile" != ""  ]; then 
    basename=`basename "$aaSeqFile"`
    rootname=${basename%.*}
    echo "Running PSIBLAST for $aaSeqFile, this may take several minutes ..."
    $blastbin/blastpgp -i $aaSeqFile -d $blastdb  -j 5 -m 9 -h 1e-3 -a 4 -o $outpath/$rootname.blast -b 10000 
    blastPath=$outpath
    AnaMSATopo3_single $rootname $aaSeqFile
elif [ "$idListFile"  != "" ]; then 
