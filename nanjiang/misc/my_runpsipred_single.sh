...
usage="
Usage:  my_runpsipred_single.sh fasta-seq-file

run psipred single single sequence prediction
...
Examples: 
    my_runpsipred_single.sh test.fasta
"
function PrintHelp()
{
    echo "$usage"
}
function MyRunPsiPredSingle() #$file#{{{
{
    local file=$1
    ...
    (for aaFile in $(cat $aaFileListFile); do 
         $psipreddir/runpsipred_single $aaFile 
      echo
    ...
    if [ "$isQuiet" != "true" ] ; then 
        echo -e "$numseq \tsequences in the file $file have been predicted by psipred_single"
    fi
