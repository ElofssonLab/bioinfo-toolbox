...
# Filename:  submit_run_hhalign_pairwise_uppmax.sh
nodename=`uname -n`
...
progname=`basename $0`
usage="
usage: $progname pairlistfile [-outpath outpath]
...
infile=
outpath=./
hhprofile=$DATADIR3/wk/MPTopo/pfamAna_refpro/hhprofile
    ...
    if [ "$numpair" == "" ]; then 
        ...
        for ((i=0;i<=numsplitted;i++)); do
            ...
            if [ -s "$splittedpairlistfile" ]; then 
                echo "\
#!/bin/bash
$DATADIR3/wk/MPTopo/src/run_hhalign_pairwise.py -l $splittedpairlistfile -hhprofile $hhprofile -outpath $suboutpath 2> $logfile 1> /dev/null" > $jobscriptfile
