...
pvaluelist <- list()
taglist <- c("IDT",  "INV|noSP", "TM2GAP|noSP", "TM2SEQ|noSP",     "SP2TM",
"Mixed", "DiffAll", "Diff without MT2SEQ and SP2TM")

for (icol in 3:10){
    pvaluevector <- c()

    if (icol <= 8){
        cnt_prokar <- as.integer(prokar[[icol]]*prokar$V10/100)
        cnt_eukar <-  as.integer(eukar[[icol]]*eukar$V10/100)   
    }else if (icol == 9){
        ...
        cnt_prokar <- as.integer(cnt_prokar*prokar$V10/100)
        cnt_eukar <- as.integer(cnt_eukar*eukar$V10/100)
    }else if (icol == 10){
        cnt_prokar <- rep(0,num_row)
        cnt_eukar <- rep(0,num_row)
        for (j in c(4,5,8)){
    ...
    total_prokar <- prokar$V10
    total_eukar <- eukar$V10
