#!/usr/bin/env python
# Filename:  build_seqid2pfamid_from_pfamscan.py
...
usage_ext="""
Description:
    Generate seqid2pfamid table file from the pfamscan result file
...
Created 2014-12-15, updated 2014-12-15, Nanjiang Shu
"""
usage_exp="""
Examples:
    %s uniref100_1.pfamscan
"""%(progname)
...
def ScanfPfamscanRecord(line):#{{{
    ...
    try:
        ...
        clanid = strs[14]
        rd = (seqid, pfamid, evalue, clanid, alnBeg, alnEnd)
        return rd
    except ValueError:
        return None
#}}}
def WriteTableLine(key, idlist, fpout):#{{{
    fpout.write("%s %d"%(key, len(idlist)))
    for idd in idlist:
        fpout.write(" %s"%(idd))
    fpout.write("\n")
#}}}
def OutputResult(seqid, li, fpout_table1, fpout_table2, fpout_domain):#{{{
    famidlist = [x[1] for x in li]
    famidlist = myfunc.uniquelist(famidlist)
    WriteTableLine(seqid, famidlist, fpout_table1)

    clanidlist = []
    for rd in li:
        clanid = rd[3]
        if clanid in ["NA", "No_clan"]:
            clanid = rd[1]
        clanidlist.append(clanid)
    clanidlist = myfunc.uniquelist(clanidlist)
    WriteTableLine(seqid, clanidlist, fpout_table2)

    domainList = []
    for rd in li:
        pfamid = rd[1]
        if pfamid[0:2] != "PB":
            domainList.append((pfamid, rd[4],rd[5]))

    fpout_domain.write("%s %d"%(seqid, len(domainList)))
    for dm in domainList:
        fpout_domain.write(" %s,%d,%d"%(dm[0], dm[1],dm[2]))
    fpout_domain.write("\n")
#}}}
def Build_seqid2pfamid(infile, g_outpath):#{{{
    ...
    seqid2clanidfile = "%s/%s.seqid2clanid"%(outpath, rootname)
    ...
    fpout_table1 = myfunc.myopen(seqid2pfamidfile, None, "w", True)
    fpout_table2 = myfunc.myopen(seqid2clanidfile, None, "w", True)
...
#         print idlist
        ...
        if len(idset_complete) > 0:
            ...
            for seqid in idlist_complete:
                try:
                    ...
                    raise
                OutputResult(seqid, li, fpout_table1, fpout_table2, fpout_domain)
            # pop up queue
    ...
    myfunc.myclose(fpout_table1)
    myfunc.myclose(fpout_table2)
    for fname in [seqid2pfamidfile, seqid2clanidfile, domainfile]:
        print  "%s output"%(fname)
