#!/bin/bash
# File name:plotCmpClass_mp1_cmpsp_5.sh 
...
Created 2012-08-31, updated 2013-05-03, Nanjiang Shu 
...
MakePlot_no_multiplot(){ #{{{  #show also number of cases as multiplot
    ...
    if [ $isTwoClass -eq 0 ];then
        ...
        ''          using 4:xtic(2)   title 'Inverted|noSP' ls 2, \
        ''          using 5:xtic(2)   title 'TM2GAP|noSP' ls 3, \
        ''          using 8:xtic(2)   title 'Mixed' ls 3, \
        ''          using 6:xtic(2)   title 'TM2SEQ' ls 4, \
        ''          using 7:xtic(2)   title 'SP2TM' ls 5
        "
    else
        plotOption1="'$dataFile' using (\$4+\$5+\$6+\$7+\$8):xtic(2)   title 'Different' ls 11 , \
        ''          using 3:xtic(2)   title 'Identical' ls 12
        "
        b1=$b1.2cls
    fi
...
set style line 6 lt 1 lw 1 lc rgb "cyan" 
set style line 7 lt 1 lw 1 lc rgb "orange"
...
plot \
    '$dataFile' using 4:xtic(2)   title 'Inverted|noSP' ls 2, \
    ''          using 5:xtic(2)   title 'TM2GAP|noSP' ls 3, \
    ''          using 8:xtic(2)   title 'Mixed' ls 6, \
    ''          using 6:xtic(2)   title 'TM2SEQ|noSP' ls 4, \
    ''          using 7:xtic(2)   title 'SP2TM' ls 5
EOF
...
#}}}
MakePlot_no_invert_multiplot(){ #{{{  #not show number of cases
    local dataFile="$1"
    local basename=`basename "$dataFile"` 
    ...
    if [ $isTwoClass -eq 0 ];then
        ...
        plotOption1="'$dataFile' using 4:xtic(2)   title 'Inverted' ls 2, \
        ''          using 5:xtic(2)   title 'TM2GAP' ls 3, \
        ''          using 8:xtic(2)   title 'Mixed' ls 6, \
        ''          using 6:xtic(2)   title 'TM2SEQ' ls 4, \
        ''          using 7:xtic(2)   title 'SP2TM' ls 5
...
if [ $isShowNumCase -eq 1 ]; then
    ...
    if [ $isPlot1 -eq 1 ];then
        if [ $isMultiplot -eq 1 ];then
            ...
            MakePlot_no_invert_multiplot $dataFile1
            MakePlot_no_invert_multiplot $dataFile2
        else
