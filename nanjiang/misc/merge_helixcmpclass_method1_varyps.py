...
Description: merge helixcmpclass table for various reliability score thresholds
...
Created 2012-09-20, updated 2012-09-21, Nanjiang Shu 
    ...
    for i in xrange(len(itemList)):
        ...
        if isEvodist:
            outfile = (outpath + os.sep + addname +
                    ".varyps.evodist%s.helixcmpclass.txt"%(itemStr))
            xlabel = "Evolutionary distance"
        else:
            outfile = (outpath + os.sep + addname +
                    ".varyps.seqidt%s.helixcmpclass.txt"%(itemStr))
            xlabel = "Sequence identity"
        try:
            fpout = open(outfile, "w")
            cnt = 0
            fpout.write("#%3s %5s %6s %6s %6s %6s\n"%("Idx", "rlty", "TM2TM",
                "TM2GAP", "TM2SEQ", "Count"))
            for dt in dataList:
                if len(dt[item]) > 0:
#                     print item, dt[item]
                    fpout.write("%-4d %5s %6.3f %6.3f %6.3f %10.0f\n"%( cnt,
                        dt['th_ps'],  dt[item][0], dt[item][1], dt[item][2],
                        dt[item][4]))
                    cnt += 1
            fpout.close()
#plotfigure
            cmd = "%s/plotHelixCmpClass_mp1_merged_varyps.sh %s -xlabel \"%s\" -outpath %s"%(rundir, outfile, xlabel, outpath)
            os.system(cmd)
            print "%s output"%outfile

            cmd = "%s/plotHelixCmpClass_mp1_merged_varyps.sh %s -xlabel \"%s\" -outpath %s -norm"%(rundir, outfile, xlabel, outpath)
            os.system(cmd)
        except IOError:
    ...
    for ps in pslist:
        if isEvodist:
            datafile = (datapath + os.sep +
                    ...
                    +"_type_all_dg100.0_gap0.0_ps%s.evodist-helixcmpclass.table.txt"%(ps))
        else:
            datafile = (datapath + os.sep +
                    "method1.TM2TM%s_TM2GAP%s"%(th_TM2TM, th_TM2GAP)
                    +"_type_all_dg100.0_gap0.0_ps%s-helixcmpclass.table.txt"%(ps))
